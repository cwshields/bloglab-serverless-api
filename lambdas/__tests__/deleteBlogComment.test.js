...
const { handler } = require("../deleteBlogComment");

describe("deleteBlogComment handler", () => {
  ...
  function mockComment(comment) {
    documentClient.get.mockImplementation(({ TableName }) => ({
      promise: () =>
        Promise.resolve(
          TableName === "blog-comments-table" ? { Item: comment } : { Item: { is_admin: false } }
        ),
    }));
  }
  ...
  test("returns 403 when a non-owner, non-admin user tries to delete someone else's comment", async () => {
    mockComment({ id: 1, blogId: 1, userId: "someone-else" });
  ...
  test("allows the comment's own author to delete it", async () => {
    mockComment({ id: 1, blogId: 1, userId: "user-1" });
  ...
  test("allows an admin to delete someone else's comment", async () => {
    documentClient.get.mockImplementation(({ TableName }) => ({
      promise: () =>
        Promise.resolve(
          TableName === "blog-comments-table"
            ? { Item: { id: 1, blogId: 1, userId: "someone-else" } }
            : { Item: { is_admin: true } }
        ),
    }));
