...
const { handler } = require("../editBlogComment");

describe("editBlogComment handler", () => {
  ...
  beforeEach(() => {
    ...
    documentClient.update.mockReturnValue({
      promise: () =>
        Promise.resolve({ Attributes: { id: 1, blogId: 1, userId: "user-1", body: "Updated!" } }),
    });
  });
  ...
  test("returns 403 when a non-owner tries to edit someone else's comment", async () => {
    documentClient.get.mockReturnValue({
      promise: () => Promise.resolve({ Item: { id: 1, blogId: 1, userId: "someone-else" } }),
    });
  ...
  test("allows the comment's own author to edit it", async () => {
    documentClient.get.mockReturnValue({
      promise: () => Promise.resolve({ Item: { id: 1, blogId: 1, userId: "user-1" } }),
    });
    ...
    expect(updateArgs.TableName).toBe("blog-comments-table");
    ...
    expect(comment).toMatchObject({ id: 1, blogId: 1, userId: "user-1", body: "Updated!" });
  });
